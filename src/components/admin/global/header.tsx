...
import { useSessionId } from "@/lib/client-auth";

export default function Header({ sessionId: propSessionId }: { sessionId?: string }) {
  const { user } = useUser();
  const router = useRouter();
  const [drawerOpen, setDrawerOpen] = useState<boolean>(false);
  const hookSessionId = useSessionId();
  const sessionId = propSessionId || hookSessionId;
      ...
      <div className="container">
        <div className="min-h-14 flex items-center gap-4 border-x px-4 md:px-6 border-b bg-white py-2">
          ...
          <div className="md:hidden">
            <button 
              className="inline-flex items-center justify-center w-8 h-8 border border-slate-200 hover:border-slate-950 rounded-md shrink-0 transition-colors duration-200 ease-linear disabled:pointer-events-none disabled:opacity-50"
              onClick={handleDrawer}
            >
              <Menu className="h-5 w-5" />
            </button>
            <nav className={`fixed top-0 left-0 ${drawerOpen ? 'md:-translate-x-full' : '-translate-x-full'} z-50 w-full max-w-xs h-screen bg-white border-r shadow-lg transition-transform duration-300 ease-in-out`}>
              <div className="grid gap-6 py-6 px-4">
                <div className="flex items-center justify-between">
                  ...
                  <button 
                    className="inline-flex items-center justify-center w-8 h-8 border border-slate-200 hover:border-slate-950 rounded-md shrink-0 md:hidden transition-colors duration-200 ease-linear disabled:pointer-events-none disabled:opacity-50"
                    onClick={handleDrawer}
                  >
                ...
                <div>
                  <ul className="grid gap-4">
                    <li>
                      <Link 
                        href="/admin" 
                        className="w-fit text-lg text-slate-950 font-medium"
                      >
                        <span>Início</span>
                      </Link>
                    </li>
                  ...
                  <ul className="grid">
                    {user?.role === 'admin' && (
                      <li>
                        <Link 
                          href="/admin/settings"
                          className="inline-block w-full text-sm rounded-md py-1.5 px-2 text-slate-950 hover:bg-slate-50 transition-colors duration-200 ease-linear"
                        >
