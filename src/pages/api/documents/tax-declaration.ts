...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if(req.method === "GET") {
    ...
    if(!parsedToken) {
      errors['global'] = { message: "Declaração IRPF não encontrada" };
    ...
    try {
      ...
      errors['global'] = { message: "Erro ao tentar encontrar declaração IRPF" };
  ...
  if(req.method === "POST") {
    ...
    try {
      const response = await fetch(`${process.env.ZAP_SIGN_API_URL}/models/create-doc/`, { 
        ...
        body: JSON.stringify({
          template_id: process.env.ZAP_SIGN_TAX_TOKEN,
      ...
      if(!response.ok) {
        errors['global'] = { message: "Erro ao tentar gerar declaração IRPF para assinatura" };
        return res.status(400).json({ success: false, errors });
      }
      ...
      console.error('Error while trying to create tax declaration in ZapSign', err);
      errors['global'] = { message: "Erro ao tentar criar declaração IRPF para assinatura" };
      return res.status(400).json({ success: false, errors });
    }
    ...
    try {
      ...
      console.error("Unexpected error during tax declaration creation:", error);
      errors['global'] = { message: "Erro ao tentar criar nova declaração IRPF, tente novamente" };
      return res.status(500).json({ success: false, errors });
    }

    return res.status(200).json({ success: true, message: 'Declaração IRPF criada com sucesso', errors, data: { sign_url: signUrl }});
  }
  ...
  if(req.method === "PUT") {
    ...
    if(!id) {
      errors['global'] = { message: "ID da declaração IRPF é inválido" };
      return res.status(400).json({ success: false, errors });
    }

    if(!token) {
      errors['global'] = { message: "Token da declaração IRPF é inválido" };
      return res.status(400).json({ success: false, errors });
    }

    let resetTax: boolean = false;
    try {
      ...
      if(!response.ok) {
        errors['global'] = { message: "Ocorreu um erro inesperado ao tentar remover declaração IRPF" };
        return res.status(500).json({ success: false, errors });
      }
      ...
      console.error(err);
      errors['global'] = { message: "Erro ao tentar deletar declaração IRPF." };
      return res.status(500).json({ success: false, errors });
    }
    ...
    if(resetTax) {
      try {
        ...
        console.error("Unexpected error while trying to reset tax declaration token:", err);
        errors['global'] = { message: "Erro ao tentar atualizar declaração IRPF" };
        return res.status(500).json({ success: false, errors });
      }
    }
  }
