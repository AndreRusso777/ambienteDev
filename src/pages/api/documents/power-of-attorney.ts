...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if(req.method === "GET") {
    ...
    if(!parsedToken) {
      errors['global'] = { message: "Procuração não encontrada" };
    ...
    try {
      ...
      errors['global'] = { message: "Erro ao tentar encontrar procuração" };
  ...
  if(req.method === "POST") {
    ...
    try {
      const response = await fetch(`${process.env.ZAP_SIGN_API_URL}/models/create-doc/`, { 
        ...
        body: JSON.stringify({
          template_id: process.env.ZAP_SIGN_POA_TOKEN,
      ...
      if(!response.ok) {
        errors['global'] = { message: "Erro ao tentar gerar procuração para assinatura" };
        return res.status(400).json({ success: false, errors });
      }
      ...
      console.error('Error while trying to create power of attorney in ZapSign', err);
      errors['global'] = { message: "Erro ao tentar criar procuração para assinatura" };
      return res.status(400).json({ success: false, errors });
    }
    ...
    try {
      ...
      console.error("Unexpected error during poa creation:", error);
      errors['global'] = { message: "Erro ao tentar criar nova procuração, tente novamente" };
      return res.status(500).json({ success: false, errors });
    }

    return res.status(200).json({ success: true, message: 'Procuração criada com sucesso', errors, data: { sign_url: signUrl }});
  }
  ...
  if(req.method === "PUT") {
    ...
    if(!id) {
      errors['global'] = { message: "ID da procuração é inválido" };
      return res.status(400).json({ success: false, errors });
    }

    if(!token) {
      errors['global'] = { message: "Token da procuração é inválido" };
      return res.status(400).json({ success: false, errors });
    }

    let resetPoa: boolean = false;
    try {
      ...
      if(!response.ok) {
        errors['global'] = { message: "Ocorreu um erro inesperado ao tentar remover procuração" };
        return res.status(500).json({ success: false, errors });
      }
      ...
      console.error(err);
      errors['global'] = { message: "Erro ao tentar deletar procuração." };
      return res.status(500).json({ success: false, errors });
    }
    ...
    if(resetPoa) {
      try {
        ...
        console.error("Unexpected error while trying to reset poa token:", err);
        errors['global'] = { message: "Erro ao tentar atualizar procuração" };
        return res.status(500).json({ success: false, errors });
      }
    }
  }
